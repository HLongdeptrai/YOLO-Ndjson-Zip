...
impl PascalVocConverter {
    ...
    fn create_voc_xml(&self, img: &ImageEntry, class_names: &HashMap<i32, String>, task: &str) -> String {
        ...
        // Root element
        writer.write_event(Event::Start(BytesStart::new("annotation"))).ok();
        ...
        // source
        writer.write_event(Event::Start(BytesStart::new("source"))).ok();
        Self::write_element(&mut writer, "database", "NDJSON Convert");
        writer.write_event(Event::End(BytesEnd::new("source"))).ok();

        // size
        writer.write_event(Event::Start(BytesStart::new("size"))).ok();
        ...
        // segmented flag
        Self::write_element(&mut writer, "segmented", if task == "segment" { "1" } else { "0" });
        ...
        if task == "segment" {
            ...
            for seg in img.get_segment_annotations() {
                ...
                writer.write_event(Event::Start(BytesStart::new("object"))).ok();
                ...
                writer.write_event(Event::Start(BytesStart::new("bndbox"))).ok();
        ...
        writer.write_event(Event::End(BytesEnd::new("annotation"))).ok();
        ...
        for (split, images) in &splits {
            ...
            if task == "classify" {
                ...
                for img in images.iter() {
                    ...
                    let xml_filename = img.file.rsplit_once('.').map(|(name, _)| name).unwrap_or(&img.file);
                    files.insert(
